...
COURSE_METRIC_BOLT_MAJOR_THREAD_DIAMETERS = [#based on max values
	-1, #0 index is not used but reduces computation
	-1,
	-1,
	2.98,#m3
...
def nutHole(size, units=MM, tolerance = +0.0001, proj = -1):
...
def boltHole(size, length, units=MM, tolerance = +0.0001, proj = -1):
    ...
    if proj == 2:
	return (translate([-capRadius/2, -capHeight])(
